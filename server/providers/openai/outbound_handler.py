...
class OpenAIOutboundHandler:
    """Handles outbound AudioRequest messages to OpenAI."""
    ...
    async def handle(self, event: ProviderInputEvent) -> None:
        """Convert and send audio payload to OpenAI over the WebSocket connection."""
        try:
            ...
            logger.debug(
                "OpenAI outbound detail commit=%s participant=%s ts=%s",
                event.commit_id,
